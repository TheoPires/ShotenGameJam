...
FText ABranch::GetInteractionText_Implementation() const
{
	return FText::FromString("Ramasser une branche");
}

void ABranch::Interact_Implementation(AActor* Interactor)
{
	if (Gatherable)
	{
		...
		{
			Character->Inventory->AddItem(FName("Branch"), Got);
		}
		K2_DestroyActor();
	}
}
